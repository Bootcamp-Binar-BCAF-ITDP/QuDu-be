...
public class DashboardService {
    ...
    public DashboardResponse getDashboard(DashboardPeriod period, LocalDate from, LocalDate to) {
        ...
        if (custom && (from == null || to == null)) {
            throw BusinessException.badRequest("from and to must be supplied together");
        }
        if (custom && from.isAfter(to)) {
            throw BusinessException.badRequest("from cannot be after to");
        }

        DateRange current = custom ? new DateRange(from, to) : resolve(period, today);
        DateRange previous = custom ? precedingWindow(current) : previous(period, current);
